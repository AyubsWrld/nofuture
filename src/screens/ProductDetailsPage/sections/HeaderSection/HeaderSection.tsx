      ...
      <nav className="inline-flex items-center gap-8 p-3 bg-[#141414] rounded-[20px] border border-solid border-[#2a2a2a]">
        ...
        <div className="inline-flex items-center justify-center gap-5 p-2.5">
          {navigationItems.map((item, index) => (
            <button
              key={index}
              className="w-fit mt-[-1.00px] [font-family:'Inter_Tight',Helvetica] font-medium text-white text-sm tracking-[0] leading-[normal] hover:opacity-80 transition-opacity cursor-pointer"
            >
              {item.label}
            </button>
          ))}
